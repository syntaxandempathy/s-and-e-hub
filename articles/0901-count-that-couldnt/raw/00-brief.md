...
**Key Sources:**
   - briefing.md
   - article-series.md - Article 1
   - tech.md
   - faq.md
   - 00-rough.md (details)
   - 0510-ai-use.py - A “generate” script: reads 4 article stages, calculates metrics (char/word counts, sentence lengths, similarity, vocab changes, semantic similarity). This is the engine for stage-level metrics.
   - 0510-chatgpt-ai-script-debugging-analysis.md - Post-mortem/analysis of multiple attempts at the “compare” script, noting regressions, lost functions, naming issues. This contextualizes the messy back half of the process.
   - 0513-chatgpt-human-contribution-metrics-charts.md - Specification + AI dialogue on turning retention into “human contribution” metrics, with JSON specs for generating charts and disclosure statements. This is the design spec for visualization + disclosure.
   - 0513-second-hand-metrics.py - A plotting script: takes the CSVs (e.g. from base_agg.csv) and generates bar charts, waterfall charts, and a summary metric. This operationalizes the JSON spec.
   - ChatGPT-Python File Analysis.md - A later session log where you actually ran 0510-ai-use.py (or a variant like import os.py) on your markup-language draft files, and then explored semantic change charts. This is evidence of the generate script in practice.
**Objective:**
Author an unbiased article based on the this brief and the activities or information specified within it. The addition of content based on assumption, extrapolation, or similar forms of fabrication are STRICTLY FORBIDDEN.